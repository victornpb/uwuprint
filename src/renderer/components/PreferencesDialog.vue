				...
				<main class="preferences-content">
					...
					<template v-else-if="activeTab === 'layout'">
						...
						<section class="settings-section"><div class="section-heading"><h3>Paper movement</h3></div>
							...
							<div class="setting-field"><span><strong>Move amount</strong><small>Pixels to feed forward or retract backward.</small></span><div class="inline-control"><input v-model.number="preferences.printer.manualFeed" type="number" min="1" max="500" /><button class="secondary" :disabled="!printerStatus.connected" @click="$emit('retract')">Retract</button><button class="secondary" :disabled="!printerStatus.connected" @click="$emit('feed')">Feed</button></div></div>
						</section>
						<section class="settings-section"><div class="section-heading"><h3>Queue settings</h3></div><label class="toggle-row"><span><strong>Pause countdown on mouse movement</strong><small>Pause auto-continue after the pointer moves more than 10px.</small></span><input v-model="preferences.queue.cancelCountdownOnMouseMove" type="checkbox" /></label></section>
					</template>
